package com.jason.yeelink.util;
...
public class YeelinkAdapter {
	...
	public static String changeGen(String apikey,int device_id,int sensor_id,String control){
		String response="";
		String order;
		try {  
		    // 首先最外层是{}，是创建一个对象  
		    ...
		    order=String.valueOf(gen);
		} catch (JSONException ex) {  
		    // 键为null或使用json不支持的数字格式(NaN, infinities)  
		    throw new RuntimeException(ex);  
		}  
		
		//order= "{ \"value\" : { \"country\" : \"china\", \"province\" : \"jiangsu\" }}";
	...
	public static String setLastGen(String apikey,String control){
		String order;
		try {  
		    // 首先最外层是{}，是创建一个对象  
		    JSONObject gen = new JSONObject();  
	...
	//泛型传感器解析
	public static String getGen(String getResult){
		String result="";
		try {  
			 JSONObject jsonObj = new JSONObject(getResult).getJSONObject("value"); 
			
			 result = jsonObj.getString("control"); 
